...
std::string getId()
{
    ...
    SHGetSpecialFolderPath( NULL, idFile, CSIDL_APPDATA, FALSE );
	 ...
	 _tcscat_s( idFile, _T("\\ScreenShottr"));
...
BOOL uploadFile(HWND hwnd, LPCTSTR fileName)
{
	const TCHAR* UPLOAD_SERVER	= _T("screenshottr.us");
	const TCHAR* UPLOAD_PATH	= _T("/upload.php?encrypted=true");
	...
	buf << sBoundary;
	buf << sCrLf;
	buf << "content-disposition: form-data; name=\"imagedata\"; filename=\"screenshottr.us\"";
	buf << sCrLf;
					...
					(DWORD) oMsg.length()))
	{
		...
		// アップロード失敗...
		MessageBox(hwnd, _T("Failed to upload - Client Timeout."), szTitle, MB_ICONERROR | MB_OK);
	}
